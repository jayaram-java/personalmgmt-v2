...
/**
* This class is used for tasklist pdf generator
*
* @author Jayaram
*/
...
public class TaskListGenerator {
	  ...
	  private static Font smallBold = new Font(Font.FontFamily.TIMES_ROMAN, 12,Font.BOLD, BaseColor.RED);
		...
		public Paragraph addTitlePage() {
			...
			preface.add(new Paragraph("Task List", catFont));

			addEmptyLine(preface, 1);

			preface.add(new Paragraph("Work is worship", smallBold));
		...
		public PdfPTable headertwo(List<EmployeeDto> employeeDtos) throws DocumentException {

			String[] header = new String[] { "Id", "Task name", "Desc" };
			...
			for (String columnHeader : header) {
				...
				headerCell.addElement(
						new Phrase(columnHeader, FontFactory.getFont(FontFactory.HELVETICA, 10, Font.BOLD)));
			...
			for (EmployeeDto employeeDto : employeeDtos) {
				...
				cell = new PdfPCell(new Phrase(employeeDto.getName(),
						FontFactory.getFont(FontFactory.HELVETICA, 10, Font.NORMAL)));
				cell.setBorderColor(BaseColor.LIGHT_GRAY);
				cell.setPadding(5);
				table.addCell(cell);

				cell = new PdfPCell(new Phrase(employeeDto.getDescription(),
						FontFactory.getFont(FontFactory.HELVETICA, 10, Font.NORMAL)));
				cell.setBorderColor(BaseColor.LIGHT_GRAY);
				cell.setPadding(5);
				table.addCell(cell);
